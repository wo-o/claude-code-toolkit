...
prompt: |
  ...
  카테고리별로 Notion 검색 (mcp__notion__search):
  - bug: 같은 증상의 과거 버그 리포트 페이지 검색 (Bugs DB 또는 incident 페이지). 있으면 링크 + 상태 인용, 없으면 "재현 정보 추가 요청" 안내
  - feature_request: Roadmap/Backlog 페이지 검색. 이미 계획에 있으면 일정 인용, 없으면 "검토 후 답변" 답변
  - usage_question: FAQ/Docs 페이지 검색, 가장 가까운 페이지 1-2개 인용

  답변 초안 200-400자 + 관련 Notion 페이지 링크 출력.
  자동 게시 X — 초안만.
```

ticket-draft-writer는 `mcp__notion__search` read-only만 가짐. **write 도구 0개**.
...
- Notion에 Bugs DB 1건 (같은 증상의 과거 리포트) + FAQ 페이지 1건

시연 1분 흐름:
1. 명령 입력 (5초)
2. ticket-classifier subagent 실행 — 분류: bug, 신뢰도 높음 (15초)
3. ticket-draft-writer subagent 실행 — Notion Bugs DB row 인용 + 답변 초안 (30초)
4. 결과 출력 + 진행자 설명: "subagent 2개가 도구 권한 분리 — Slack read만 / Notion search만. 자동 게시 X" (10초)
...
- **draft-writer가 카테고리 무관 검색**: 카테고리에 따라 검색 키워드와 대상 Notion 영역이 달라야 함. classifier 결과를 prompt에 명시적으로 분기 요구
...
- 미해결 case는 "현재 검토 중입니다. <Notion link>에서 진행 상황 확인 가능합니다" 식으로 honest
